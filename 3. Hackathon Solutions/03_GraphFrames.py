...
# MAGIC 
# MAGIC See https://graphframes.github.io/graphframes/docs/_site/user-guide.html#message-passing-via-aggregatemessages
...
def get_sku_for_raw(gx):
  ...
  vertices_with_agg_messages = spark.createDataFrame([],schema)
...
demand_df = spark.read.table(f"{db_name}.part_level_demand_with_forecasts")
sku_mapper = spark.read.table(f"{db_name}.sku_mapper")
bom = spark.read.table(f"{db_name}.bom")
...
display(spark.sql(f"select distinct SKU from {db_name}.part_level_demand_with_forecasts"))
...
demand_raw_df.write \
.mode("overwrite") \
.saveAsTable(f"{db_name}.forecast_raw")

# COMMAND ----------

display(spark.sql(f"SELECT * FROM {db_name}.forecast_raw"))
